...
**Copy your local DB + product image URLs to production:** see **[deploy/SYNC-LOCAL-TO-VPS.md](./SYNC-LOCAL-TO-VPS.md)** (`pg_dump` / `pg_restore`, or `npm run db:seed` for demo-only). **Create or reset a live admin:** same doc — `ADMIN_EMAIL` + `ADMIN_PASSWORD` + `npm run db:create-admin` on the server.
